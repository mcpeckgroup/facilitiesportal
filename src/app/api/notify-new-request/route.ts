...
export async function POST(req: Request) {
  try {
    ...
    if (!apiKey) {
      console.error("[notify-new-request] RESEND_API_KEY missing");
      return new Response("Missing RESEND_API_KEY", { status: 500 });
    }
    if (!from) {
      console.error("[notify-new-request] NOTIFY_FROM missing");
      return new Response("Missing NOTIFY_FROM", { status: 500 });
    }
    ...
    if (!res.ok) {
      console.error("[notify-new-request] Resend error", {
        status: res.status,
    ...
    console.log("[notify-new-request] Email accepted by Resend");
